...
function onLoad() {
	var sky:BGSprite = new BGSprite('backgrounds/tankSky', -400, -400, 0, 0);
	add(sky);

	if (!ClientPrefs.lowQuality) {
		var clouds:BGSprite = new BGSprite('backgrounds/tankClouds', FlxG.random.int(-700, -100), FlxG.random.int(-20, 20), 0.1, 0.1);
		clouds.active = true;
		clouds.velocity.x = FlxG.random.float(5, 15);
		add(clouds);

		var mountains:BGSprite = new BGSprite('backgrounds/tankMountains', -300, -20, 0.2, 0.2);
		mountains.setGraphicSize(Std.int(1.2 * mountains.width));
		mountains.updateHitbox();
		add(mountains);

		var buildings:BGSprite = new BGSprite('backgrounds/tankBuildings', -200, 0, 0.3, 0.3);
		buildings.setGraphicSize(Std.int(1.1 * buildings.width));
		buildings.updateHitbox();
		add(buildings);
	}

	var ruins:BGSprite = new BGSprite('backgrounds/tankRuins', -200, 0, .35, .35);
	ruins.setGraphicSize(Std.int(1.1 * ruins.width));
	ruins.updateHitbox();
	add(ruins);

	if (!ClientPrefs.lowQuality) {
		var smokeLeft:BGSprite = new BGSprite('backgrounds/smokeLeft', -200, -100, 0.4, 0.4, ['SmokeBlurLeft instance 1'], true);
		add(smokeLeft);
		var smokeRight:BGSprite = new BGSprite('backgrounds/smokeRight', 1100, -100, 0.4, 0.4, ['SmokeRight instance 1'], true);
		add(smokeRight);

		tankWatchtower = new BGSprite('backgrounds/tankWatchtower', 100, 50, 0.5, 0.5, ['watchtower gradient color instance 1']);
		add(tankWatchtower);
	}

	tankGround = new BGSprite('backgrounds/tankRolling', 300, 300, 0.5, 0.5, ['BG tank w lighting instance 1'], true);
	add(tankGround);

	tankmanRun = new FlxTypedGroup();
	add(tankmanRun);

	var ground:BGSprite = new BGSprite('backgrounds/tankGround', -420, -150);
	ground.setGraphicSize(Std.int(1.15 * ground.width));
	ground.updateHitbox();
	add(ground);
	moveTank();

	var tank0 = new BGSprite('backgrounds/tank0', -500, 650, 1.7, 1.5, ['fg tankhead far right instance 1']);
	tank0.zIndex = 999;
	add(tank0);

	var tank2 = new BGSprite('backgrounds/tank2', 450, 940, 1.5, 1.5, ['foreground man 3 instance 1']);
	tank2.zIndex = 999;
	add(tank2);

	var tank4 = new BGSprite('backgrounds/tank5', 1620, 700, 1.5, 1.5, ['fg tankhead far right instance 1']);
	tank4.zIndex = 999;
	add(tank4);
	...
	if (!ClientPrefs.lowQuality) {
		var tank1 = new BGSprite('backgrounds/tank1', -300, 750, 2, 0.2, ['fg tankhead 5 instance 1']);
		tank1.zIndex = 999;
		add(tank1);
		boppers.push(tank1);

		var tank3 = new BGSprite('backgrounds/tank4', 1300, 900, 1.5, 1.5, ['fg tankman bobbin 3 instance 1']);
		tank3.zIndex = 999;
		add(tank3);
		boppers.push(tank3);

		var tank5 = new BGSprite('backgrounds/tank3', 1300, 1200, 3.5, 2.5, ['fg tankhead 4 instance 1']);
		tank5.zIndex = 999;
		add(tank5);
		boppers.push(tank5);
	}

	// foreground.add(new BGSprite('backgrounds/tank0', -500, 650, 1.7, 1.5, ['fg']));
	// if(!ClientPrefs.lowQuality) foreground.add(new BGSprite('backgrounds/tank1', -300, 750, 2, 0.2, ['fg']));
	// foreground.add(new BGSprite('backgrounds/tank2', 450, 940, 1.5, 1.5, ['foreground']));
	// if(!ClientPrefs.lowQuality) foreground.add(new BGSprite('backgrounds/tank4', 1300, 900, 1.5, 1.5, ['fg']));
	// foreground.add(new BGSprite('backgrounds/tank5', 1620, 700, 1.5, 1.5, ['fg']));
	// if(!ClientPrefs.lowQuality) foreground.add(new BGSprite('backgrounds/tank3', 1300, 1200, 3.5, 2.5, ['fg']));
...
function updatePicoChart() {
	if (picoAnims.length != 0 && picoAnims[0].time <= Conductor.songPosition) {
		...
		if (data.length == 0 || data.length < holdingTime)
			picoAnims.shift();
	}
}
