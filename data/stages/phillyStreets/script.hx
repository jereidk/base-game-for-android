...
function onLoad()
{
	scrollingSky = new FlxTiledSprite(Paths.image('backgrounds/phillyStreets/phillySkybox'), 2922, 850, true, false);
	scrollingSky.setPosition(-1200, -900);
	scrollingSky.scrollFactor.set(0.1, 0.1);
	scrollingSky.scale.set(0.65, 0.65);
	add(scrollingSky);
	
	var skyline = new FlxSprite(-800, -800).loadGraphic(Paths.image('backgrounds/phillyStreets/phillySkyline'));
	skyline.scrollFactor.set(0.2, 0.2);
	add(skyline);
	
	var bgCity = new FlxSprite(250, -450).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyForegroundCity'));
	bgCity.scrollFactor.set(0.45, 0.45);
	add(bgCity);
	
	var hLights = new FlxSprite(-800, -285).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyHighwayLights'));
	add(hLights);
	
	var hLightsMap = new FlxSprite(-800, -285).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyHighwayLights_lightmap'));
	hLightsMap.blend = BlendMode.ADD;
	add(hLightsMap);
	
	var highway = new FlxSprite(-800, -400).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyHighway'));
	add(highway);
	
	var buildingFront = new FlxSprite(1050, -600).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyConstruction'));
	buildingFront.scrollFactor.set(0.6, 0.6);
	buildingFront.scale.set(1.3, 1.3);
	buildingFront.updateHitbox();
	add(buildingFront);
	
	var smog = new FlxSprite(-600, -400).loadGraphic(Paths.image('backgrounds/phillyStreets/phillySmog'));
	add(smog);
	
	trafficLight = new FlxSprite(1200);
	trafficLight.frames = Paths.getSparrowAtlas('backgrounds/phillyStreets/phillyTraffic');
	...
	var lightmap = new FlxSprite(1200, -5).loadGraphic(Paths.image('backgrounds/phillyStreets/phillyTraffic_lightmap'));
	...
	for (i in 0...4)
	{
		...
		car.frames = Paths.getSparrowAtlas('backgrounds/phillyStreets/phillyCars');
	...
	var ground = new FlxSprite().loadGraphic(Paths.image('backgrounds/phillyStreets/phillyForeground'));
	ground.screenCenter();
	add(ground);
